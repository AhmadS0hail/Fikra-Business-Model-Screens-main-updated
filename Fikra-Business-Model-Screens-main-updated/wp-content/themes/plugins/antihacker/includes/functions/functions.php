...
if( $antihacker_is_human != 0) {
    ...
    // add_action('wp_ajax_nopriv_antihacker_get_ajax_data', 'antihacker_get_ajax_data');
...
function antihacker_populate_rules2()
{
  global $wpdb;
  require_once(ABSPATH . 'wp-admin/includes/upgrade.php');
    ...
    $r = $wpdb->get_var("SELECT COUNT(*) FROM `$table_name`");
  ...
  //  die( var_export($fhandle, true));

    if (!$fhandle)
        return;

       /*
        INSERT INTO `wp_ah_rules` (`id`, `name`, `strings`, `cond`, `descri`, `autor`, `url`, `obs`, `flag`) 
        VALUES(29, 
        'Php_Trojan_MSShellcode_82', 
        'JGEwID0geyA2OTY2MjAyODIxNjk3MzczNjU3NDI4MjQ0NzRjNGY0MjQxNGM1MzViMjc2MzY4NjE2ZTZlNjU2YzczMjc1ZDI5MjkyMDdiMjAyNDQ3NGM0ZjQyNDE0YzUzNWIyNzYzNjg2MTZlNmU2NTZjNzMyNzVkMjAzZDIwNjE3MjcyNjE3OSB9\n', 
        '$a0', '', '', '', '', '');
        */      
       //die('1');
    ...
    while (!feof($fhandle)) {
        ...
        //$line = sanitize_text_field(fgets($fhandle));
        $line = fgets($fhandle);


        // INSERT INTO `wp_ah_rules` (`id`, `name`, `strings`, `cond`, `descri`, `autor`, `url`, `obs`, `flag`) VALUES(1, 'ajaxcommand', 'JGEgPSAiQWpheCBDb21tYW5kIFNoZWxsIGJ5Ig==\nJGIgPSAiYSBocmVmPWh0dHA6Ly93d3cuaXJvbndhcmV6LmluZm8i\nJGMgPSAiJ0NsZWFyIEhpc3RvcnknID0+ICdDbGVhckhpc3RvcnkoKSci\nJGQgPSAiZm9yIHNvbWUgZWhoLi4uaGVscCI=\n', 'any of them', ' Ajax Command shell', '', ' https://github.com/tennc/webshell/blob/master/xakep-shells/PHP/Ajax_PHP%20Command%20Shell.php.txt', '', '');

        $mylines = explode(',', $line);

        /*
        sanitized above...
        for($i =0; $i < count($mylines),$i++){

            $mylines[$i] = sanitize_text_field($mylines[$i]);
       ...
       /* $r = $wpdb->get_results($wpdb->prepare(
            "INSERT INTO `$table_name`
            (`id`, `name`, `strings`, `cond`, `descri`, `autor`, `url`, `obs`, `flag`) 
            VALUES (%s, %s , %s, %s, %s, %s, %s, %s, %s)", 
            $mylines[0], $mylines[1], $mylines[2], $mylines[3],$mylines[4], $mylines[5], $mylines[6], $mylines[7], $mylines[8] ));
*/

            $r = $wpdb->get_var("SELECT COUNT(*) FROM `$table_name` WHERE name = $mylines[1]");
            
            if($r < 1) {
                $r = $wpdb->get_results("INSERT INTO `$table_name`
                    (`id`, `name`, `strings`, `cond`, `descri`, `autor`, `url`, `obs`, `flag`) 
                    VALUES (
                    '$mylines[0]', 
                    '$mylines[1]',' 
...
function antihacker_check_memory()
{
    ...
    // 2023
    $antihacker_memory['free'] = $antihacker_memory['wp_limit'] - $antihacker_memory['usage'];
...
function antihacker_isourserver()
{
    ...
    $server_ip = sanitize_text_field($_SERVER['SERVER_ADDR']);
    if ($server_ip == $antihackerip or $antihackerip == '173.212.229.226')
        return true;
...
*/
    ...
    for ($i = 0; $i < count($mysearch); $i++) {
        if (stripos($ua, $mysearch[$i]) !== false) {
            ...
            if(isset($mysearch1[$i])) {
                if(gettype($mysearch1[$i]) == 'string') {
                    if (stripos($host, $mysearch1[$i]) !== false) {
                        return true;
                    }
                }
...
function antihacker_errors()
{
	...
	//2023
		if(antihacker_javascript_errors_today(1) or antihacker_errors_today(1)){
	...
	foreach ($antihacker_folders as $antihacker_folder) {
		foreach (glob($antihacker_folder) as $antihacker_filename) {
			if (strpos($antihacker_filename, 'backup') != true) {
				...
				echo '</strong>';
				$antihacker_count++;
				$marray = antihacker_read_file($antihacker_filename, 1000);
	...
	echo "<p>" . esc_attr(__("Log Files found", "wp-memory")) . ": " . esc_attr($antihacker_count) . "</p>";
}
